poolB = document.getElementsByClassName("poolB");
if (poolB.length == 1){
    numJoueurB = document.getElementsByClassName("numJoueurB");
    nombreJoueurB = document.getElementById("poolB").attributes.nombre.value;
    nombreJoueurB = parseInt(nombreJoueurB);
    tourB = document.getElementsByClassName("tourB");
    infoTabB = document.getElementsByClassName("infoTabB");
    nombreTourB = infoTabB.length;
    numJoueurB1 = document.getElementsByClassName("numJoueurB1");
    numJoueurB2 = document.getElementsByClassName("numJoueurB2");
    numJoueurB3 = document.getElementsByClassName("numJoueurB3");
    numJoueurB4 = document.getElementsByClassName("numJoueurB4");
    numJoueurB5 = document.getElementsByClassName("numJoueurB5");
    numJoueurB6 = document.getElementsByClassName("numJoueurB6");
    numJoueurB7 = document.getElementsByClassName("numJoueurB7");

    finaliste1A = document.getElementById("1A");
    finaliste1B = document.getElementById("1B");
    finaliste2A = document.getElementById("2A");
    finaliste2B = document.getElementById("2B");

    finaliste1 = document.getElementById("1");
    finaliste2 = document.getElementById("2");
    finaliste3 = document.getElementById("3");
    finaliste4 = document.getElementById("4");
    tabJoueurB = [];

    for (i = 0; i < nombreJoueurB; i++) {
        tabJoueurB[i] = "score" + numJoueurB[i].attributes.joueur.value;
    }
}
numJoueurA = document.getElementsByClassName("numJoueurA");
nombreJoueurA = document.getElementById("poolA").attributes.nombre.value;
nombreJoueurA = parseInt(nombreJoueurA);
tourA = document.getElementsByClassName("tourA");
infoTabA = document.getElementsByClassName("infoTabA");
nombreTourA = infoTabA.length;
numJoueurA1 = document.getElementsByClassName("numJoueurA1");
numJoueurA2 = document.getElementsByClassName("numJoueurA2");
numJoueurA3 = document.getElementsByClassName("numJoueurA3");
numJoueurA4 = document.getElementsByClassName("numJoueurA4");
numJoueurA5 = document.getElementsByClassName("numJoueurA5");
numJoueurA6 = document.getElementsByClassName("numJoueurA6");
numJoueurA7 = document.getElementsByClassName("numJoueurA7");

scoreTotal = document.getElementsByClassName("scoreTotal");
scoreJoueur=document.getElementsByClassName("score");
classement=document.getElementsByClassName("classement");

tabJoueur = [];

for (i = 0; i < nombreJoueurA; i++) {
    tabJoueur[i] = "score" + numJoueurA[i].attributes.joueur.value;
}

/**pour les input du second tour de la pool A */
for (let b = 0; b < numJoueurA1.length; b++) {
    if (numJoueurA1[b].value != "")
    {
        numJoueurA1[b].style.background = "rgb(151, 238, 151)";
        numJoueurA1[b].setAttribute("class","numJoueurA numJoueurA1 score0 rempliA1");
    }  
    if (numJoueurA2[b].value != "")
    {
        numJoueurA2[b].style.background = "rgb(151, 238, 151)";
        numJoueurA2[b].setAttribute("class","numJoueurA numJoueurA2 score1 rempliA2");
    }  
    if (numJoueurA3[b].value != "")
    {
        numJoueurA3[b].style.background = "rgb(151, 238, 151)";
        numJoueurA3[b].setAttribute("class","numJoueurA numJoueurA3 score2 rempliA3");
    }
    if (numJoueurA4.length>0 && numJoueurA4[b].value != "")
    {
        numJoueurA4[b].style.background = "rgb(151, 238, 151)";
        numJoueurA4[b].setAttribute("class","numJoueurA numJoueurA4 score3 rempliA4");
    }
    if (numJoueurA5.length>0 && numJoueurA5[b].value != "")
    {
        numJoueurA5[b].style.background = "rgb(151, 238, 151)";
        numJoueurA5[b].setAttribute("class","numJoueurA numJoueurA5 score4 rempliA5");
    }
    if (numJoueurA6.length>0 && numJoueurA6[b].value != "")
    {
        numJoueurA6[b].style.background = "rgb(151, 238, 151)";
        numJoueurA6[b].setAttribute("class","numJoueurA numJoueurA6 score5 rempliA6");
    }
    if (numJoueurA7.length>0 && numJoueurA7[b].value != "")
    {
        numJoueurA7[b].style.background = "rgb(151, 238, 151)";
        numJoueurA7[b].setAttribute("class","numJoueurA numJoueurA7 score6 rempliA7");
    }    
}


rempliA1=document.getElementsByClassName("rempliA1");
rempliA2=document.getElementsByClassName("rempliA2");
rempliA3=document.getElementsByClassName("rempliA3");
rempliA4=document.getElementsByClassName("rempliA4");
rempliA5=document.getElementsByClassName("rempliA5");
rempliA6=document.getElementsByClassName("rempliA6");
rempliA7=document.getElementsByClassName("rempliA7");

if (rempliA1.length == nombreJoueurA && tourA[0].style.background != "rgb(3, 90, 3)") {
    tourA[0].style.background = "rgb(3, 90, 3)";
    infoTabA[0].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA2.length == nombreJoueurA && tourA[1].style.background != "rgb(3, 90, 3)") {
    tourA[1].style.background = "rgb(3, 90, 3)";
    infoTabA[1].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA3.length == nombreJoueurA && tourA[2].style.background != "rgb(3, 90, 3)") {
    tourA[2].style.background = "rgb(3, 90, 3)";
    infoTabA[2].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA4.length == nombreJoueurA && tourA[3].style.background != "rgb(3, 90, 3)") {
    tourA[3].style.background = "rgb(3, 90, 3)";
    infoTabA[3].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA5.length == nombreJoueurA && tourA[4].style.background != "rgb(3, 90, 3)") {
    tourA[4].style.background = "rgb(3, 90, 3)";
    infoTabA[4].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA6.length == nombreJoueurA && tourA[5].style.background != "rgb(3, 90, 3)") {
    tourA[5].style.background = "rgb(3, 90, 3)";
    infoTabA[5].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliA7.length == nombreJoueurA && tourA[6].style.background != "rgb(3, 90, 3)") {
    tourA[6].style.background = "rgb(3, 90, 3)";
    infoTabA[6].style.border = "rgb(115, 255, 0) solid 2px";
}
if(tourA[0].style.background != "rgb(3, 90, 3)")
    numJoueurA1[(numJoueurA1.length-1)].focus();
else if (tourA[1].style.background != "rgb(3, 90, 3)")
    numJoueurA2[(numJoueurA1.length-1)].focus();
else if (tourA[2].style.background != "rgb(3, 90, 3)")
    numJoueurA3[(numJoueurA1.length-1)].focus();
else if (tourA.length>3 && tourA[3].style.background != "rgb(3, 90, 3)")
    numJoueurA4[(numJoueurA1.length-1)].focus();
else if (tourA.length>4 && tourA[4].style.background != "rgb(3, 90, 3)")
    numJoueurA5[(numJoueurA1.length-1)].focus();
else if (tourA.length>5 && tourA[5].style.background != "rgb(3, 90, 3)")
    numJoueurA6[(numJoueurA1.length-1)].focus();
else if (tourA.length>6 && tourA[6].style.background != "rgb(3, 90, 3)")
    numJoueurA7[(numJoueurA1.length-1)].focus();
tot = [];
score1 = [];
score2 = [];
score3 = [];
score4 = [];
score5 = [];
score6 = [];
score7 = [];
for (i = 0; i < numJoueurA1.length; i++) {
    if (numJoueurA1[i].value != '')
        score1[i] = parseInt(numJoueurA1[i].value);
    else
        score1[i] = 0
}
for (i = 0; i < numJoueurA2.length; i++) {
    if (numJoueurA2[i].value != '')
        score2[i] = parseInt(numJoueurA2[i].value);
    else
        score2[i] = 0
}
for (i = 0; i < numJoueurA3.length; i++) {
    if (numJoueurA3[i].value != '')
        score3[i] = parseInt(numJoueurA3[i].value);
    else
        score3[i] = 0
}
for (i = 0; i < numJoueurA4.length; i++) {
    if (numJoueurA4[i].value != '')
        score4[i] = parseInt(numJoueurA4[i].value);
    else
        score4[i] = 0
}
for (i = 0; i < numJoueurA5.length; i++) {
    if (numJoueurA5[i].value != '')
        score5[i] = parseInt(numJoueurA5[i].value);
    else
        score5[i] = 0
}
for (i = 0; i < numJoueurA6.length; i++) {
    if (numJoueurA6[i].value != '')
        score6[i] = parseInt(numJoueurA6[i].value);
    else
        score6[i] = 0
}
for (i = 0; i < numJoueurA7.length; i++) {
    if (numJoueurA7[i].value != '')
        score7[i] = parseInt(numJoueurA7[i].value);
    else
        score7[i] = 0
}
 /***********************************************************************************************************************************************************************************************************************************
         * ********************************************************* gestion des scores */
        switch (nombreJoueurA) {
            /***************************************************************************************************************case 3 */
            case 3:
                tot[0] = score1[0] + score2[0] + score3[2];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[0];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[1];
                scoreTotal[2].innerHTML = tot[2];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = 0 ;
                    score4 = 0 ;
                    score5 = 0 ;
                    score6 = 0 ;
                    score7 = 0 ;  
                }
                break;
                /***************************************************************************************************************case 4 */
            case 4:
                tot[0] = score1[0] + score2[0] + score3[0];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[2];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[3];
                scoreTotal[2].innerHTML = tot[2];
                tot[3] = score1[3] + score2[3] + score3[1];
                scoreTotal[3].innerHTML = tot[3];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = parseInt(scoreTotal[3].innerHTML);
                    score4 = 0 ;
                    score5 = 0 ;
                    score6 = 0 ;
                    score7 = 0 ;  
                }
                break;
                /***************************************************************************************************************case 5 */
            case 5:
                tot[0] = score1[0] + score2[0] + score3[0] + score4[0] + score5[4];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[2] + score4[4] + score5[0];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[4] + score4[2] + score5[1];
                scoreTotal[2].innerHTML = tot[2];
                tot[3] = score1[3] + score2[4] + score3[3] + score4[1] + score5[2];
                scoreTotal[3].innerHTML = tot[3];
                tot[4] = score1[4] + score2[3] + score3[1] + score4[3] + score5[3];
                scoreTotal[4].innerHTML = tot[4];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)" && tourA[3].style.background == "rgb(3, 90, 3)" && tourA[4].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = parseInt(scoreTotal[3].innerHTML);
                    score4 = parseInt(scoreTotal[4].innerHTML);
                    score5 = 0 ;
                    score6 = 0 ;
                    score7 = 0 ;
                }
                break;
                /***************************************************************************************************************case 6 */
            case 6:
                tot[0] = score1[0] + score2[0] + score3[0] + score4[0] + score5[4];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[2] + score4[4] + score5[0];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[4] + score4[2] + score5[1];
                scoreTotal[2].innerHTML = tot[2];
                tot[3] = score1[3] + score2[4] + score3[3] + score4[1] + score5[2];
                scoreTotal[3].innerHTML = tot[3];
                tot[4] = score1[4] + score2[3] + score3[1] + score4[3] + score5[3];
                scoreTotal[4].innerHTML = tot[4];
                tot[5] = score1[5] + score2[5] + score3[5] + score4[5] + score5[5];
                scoreTotal[5].innerHTML = tot[5];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)" && tourA[3].style.background == "rgb(3, 90, 3)" && tourA[4].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = parseInt(scoreTotal[3].innerHTML);
                    score4 = parseInt(scoreTotal[4].innerHTML);
                    score5 = parseInt(scoreTotal[5].innerHTML);
                    score6 = 0 ;
                    score7 = 0 ;
                }
                break;
                /***************************************************************************************************************case 7 */
            case 7:
                tot[0] = score1[0] + score2[0] + score3[0] + score4[0] + score5[0] + score6[0] + score7[6];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[2] + score4[4] + score5[2] + score6[6] + score7[0];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[3] + score4[2] + score5[6] + score6[2] + score7[2];
                scoreTotal[2].innerHTML = tot[2];
                tot[3] = score1[3] + score2[4] + score3[1] + score4[6] + score5[3] + score6[4] + score7[4];
                scoreTotal[3].innerHTML = tot[3];
                tot[4] = score1[4] + score2[3] + score3[6] + score4[1] + score5[4] + score6[3] + score7[5];
                scoreTotal[4].innerHTML = tot[4];
                tot[5] = score1[5] + score2[6] + score3[4] + score4[5] + score5[1] + score6[5] + score7[3];
                scoreTotal[5].innerHTML = tot[5];
                tot[6] = score1[6] + score2[5] + score3[5] + score4[3] + score5[5] + score6[1] + score7[1];
                scoreTotal[6].innerHTML = tot[6];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)" && tourA[3].style.background == "rgb(3, 90, 3)" && tourA[4].style.background == "rgb(3, 90, 3)"
                    && tourA[5].style.background == "rgb(3, 90, 3)" && tourA[6].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = parseInt(scoreTotal[3].innerHTML);
                    score4 = parseInt(scoreTotal[4].innerHTML);
                    score5 = parseInt(scoreTotal[5].innerHTML);
                    score6 = parseInt(scoreTotal[6].innerHTML);
                    score7 = 0 ;
                }
                break;
                /***************************************************************************************************************case 8 */
            case 8:
                tot[0] = score1[0] + score2[0] + score3[0] + score4[0] + score5[0] + score6[0] + score7[6];
                scoreTotal[0].innerHTML = tot[0];
                tot[1] = score1[1] + score2[2] + score3[2] + score4[4] + score5[2] + score6[6] + score7[0];
                scoreTotal[1].innerHTML = tot[1];
                tot[2] = score1[2] + score2[1] + score3[3] + score4[2] + score5[6] + score6[2] + score7[2];
                scoreTotal[2].innerHTML = tot[2];
                tot[3] = score1[3] + score2[4] + score3[1] + score4[6] + score5[3] + score6[4] + score7[4];
                scoreTotal[3].innerHTML = tot[3];
                tot[4] = score1[4] + score2[3] + score3[6] + score4[1] + score5[4] + score6[3] + score7[5];
                scoreTotal[4].innerHTML = tot[4];
                tot[5] = score1[5] + score2[6] + score3[4] + score4[5] + score5[1] + score6[5] + score7[3];
                scoreTotal[5].innerHTML = tot[5];
                tot[6] = score1[6] + score2[5] + score3[5] + score4[3] + score5[5] + score6[1] + score7[1];
                scoreTotal[6].innerHTML = tot[6];
                tot[7] = score1[7] + score2[7] + score3[7] + score4[7] + score5[7] + score6[7] + score7[7];
                scoreTotal[7].innerHTML = tot[7];
                if (tourA[0].style.background == "rgb(3, 90, 3)" && tourA[1].style.background == "rgb(3, 90, 3)" && tourA[2].style.background == "rgb(3, 90, 3)" && tourA[3].style.background == "rgb(3, 90, 3)" && tourA[4].style.background == "rgb(3, 90, 3)"
                    && tourA[5].style.background == "rgb(3, 90, 3)" && tourA[6].style.background == "rgb(3, 90, 3)") {
                    score0 = parseInt(scoreTotal[0].innerHTML);
                    score1 = parseInt(scoreTotal[1].innerHTML);
                    score2 = parseInt(scoreTotal[2].innerHTML);
                    score3 = parseInt(scoreTotal[3].innerHTML);
                    score4 = parseInt(scoreTotal[4].innerHTML);
                    score5 = parseInt(scoreTotal[5].innerHTML);
                    score6 = parseInt(scoreTotal[6].innerHTML);
                    score7 = parseInt(scoreTotal[7].innerHTML);
                }
                break;
        }

/***********************************************************************************************************************************************************************************************************************
* *****************************************************************************************************************************comparaison des scores*/
        if (typeof(score0) !== 'undefined') {
        {
            /***********************************score 0 est le plus grand */
        if (score0 >= score1 && score0 >= score2 && score0 >= score3 && score0 >= score4 && score0 >= score5 && score0 >= score6 && score0 >= score7) {
            id = "1" + tabJoueur[0];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[0].style.background="green";
            classement[0].innerHTML="1er"; 
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score1 >= score2 && score1 >= score3 && score1 >= score4 && score1 >= score5 && score1 >= score6 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score1 && score2 >= score3 && score2 >= score4 && score2 >= score5 && score2 >= score6 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score1 && score3 >= score2 && score3 >= score4 && score3 >= score5 && score3 >= score6 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score1 && score4 >= score2 && score4 >= score3 && score4 >= score5 && score4 >= score6 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score1 && score5 >= score2 && score5 >= score3 && score5 >= score4 && score5 >= score6 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else if (score6 >= score1 && score6 >= score2 && score6 >= score3 && score6 >= score4 && score6 >= score5 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
           
        }
        /***********************************score 1 est le plus grand */
        else if (score1 >= score0 && score1 >= score2 && score1 >= score3 && score1 >= score4 && score1 >= score5 && score1 >= score6 && score1 >= score7) {
            id = "1" + tabJoueur[1];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[1].style.background="green";
            classement[1].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score0 >= score2 && score0 >= score3 && score0 >= score4 && score0 >= score5 && score0 >= score6 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score3 && score2 >= score4 && score2 >= score5 && score2 >= score6 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score2 && score3 >= score4 && score3 >= score5 && score3 >= score6 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score2 && score4 >= score3 && score4 >= score5 && score4 >= score6 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score2 && score5 >= score3 && score5 >= score4 && score5 >= score6 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else if (score6 >= score0 && score6 >= score2 && score6 >= score3 && score6 >= score4 && score6 >= score5 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 2 est le plus grand */
        else if (score2 >= score0 && score2 >= score1 && score2 >= score3 && score2 >= score4 && score2 >= score5 && score2 >= score6 && score2 >= score7) {
            id = "1" + tabJoueur[2];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[2].style.background="green";
            classement[2].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {    
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score0 >= score1 && score0 >= score3 && score0 >= score4 && score0 >= score5 && score0 >= score6 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score3 && score1 >= score4 && score1 >= score5 && score1 >= score6 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score1 && score3 >= score4 && score3 >= score5 && score3 >= score6 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score1 && score4 >= score3 && score4 >= score5 && score4 >= score6 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score1 && score5 >= score3 && score5 >= score4 && score5 >= score6 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else if (score6 >= score0 && score6 >= score1 && score6 >= score3 && score6 >= score4 && score6 >= score5 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 3 est le plus grand */
        else if (score3 >= score0 && score3 >= score1 && score3 >= score2 && score3 >= score4 && score3 >= score5 && score3 >= score6 && score3 >= score7) {
            id = "1" + tabJoueur[3];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[3].style.background="green";
            classement[3].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }  
            if (score0 >= score1 && score0 >= score2 && score0 >= score4 && score0 >= score5 && score0 >= score6 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score2 && score1 >= score4 && score1 >= score5 && score1 >= score6 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score1 && score2 >= score4 && score2 >= score5 && score2 >= score6 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score1 && score4 >= score2 && score4 >= score5 && score4 >= score6 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score1 && score5 >= score2 && score5 >= score4 && score5 >= score6 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else if (score6 >= score0 && score6 >= score1 && score6 >= score2 && score6 >= score4 && score6 >= score5 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 4 est le plus grand */
        else if (score4 >= score0 && score4 >= score1 && score4 >= score2 && score4 >= score3 && score4 >= score5 && score4 >= score6 && score4 >= score7) {
            id = "1" + tabJoueur[4];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[4].style.background="green";
            classement[4].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score0 >= score1 && score0 >= score2 && score0 >= score3 && score0 >= score5 && score0 >= score6 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score2 && score1 >= score3 && score1 >= score5 && score1 >= score6 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score1 && score2 >= score3 && score2 >= score5 && score2 >= score6 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score1 && score3 >= score2 && score3 >= score5 && score3 >= score6 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score1 && score5 >= score2 && score5 >= score3 && score5 >= score6 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else if (score6 >= score0 && score6 >= score1 && score6 >= score2 && score6 >= score3 && score6 >= score5 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 5 est le plus grand */
        else if (score5 >= score0 && score5 >= score1 && score5 >= score2 && score5 >= score3 && score5 >= score4 && score5 >= score6 && score5 >= score7) {
            id = "1" + tabJoueur[5];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[5].style.background="green";
            classement[5].innerHTML="1er"; 
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value); 
            } 
            if (score0 >= score1 && score0 >= score2 && score0 >= score3 && score0 >= score4 && score0 >= score6 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score2 && score1 >= score3 && score1 >= score4 && score1 >= score6 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score1 && score2 >= score3 && score2 >= score4 && score2 >= score6 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score1 && score3 >= score2 && score3 >= score4 && score3 >= score6 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score1 && score4 >= score2 && score4 >= score3 && score4 >= score6 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score6 >= score0 && score6 >= score1 && score6 >= score2 && score6 >= score3 && score6 >= score4 && score6 >= score7) {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 6 est le plus grand */
        else if (score6 >= score0 && score6 >= score1 && score6 >= score2 && score6 >= score3 && score6 >= score4 && score6 >= score5 && score6 >= score7) {
            id = "1" + tabJoueur[6];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[6].style.background="green";
            classement[6].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score0 >= score1 && score0 >= score2 && score0 >= score3 && score0 >= score4 && score0 >= score5 && score0 >= score7) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score2 && score1 >= score3 && score1 >= score4 && score1 >= score5 && score1 >= score7) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score1 && score2 >= score3 && score2 >= score4 && score2 >= score5 && score2 >= score7) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score1 && score3 >= score2 && score3 >= score4 && score3 >= score5 && score3 >= score7) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score1 && score4 >= score2 && score4 >= score3 && score4 >= score5 && score4 >= score7) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score1 && score5 >= score2 && score5 >= score3 && score5 >= score4 && score5 >= score7) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[7];
                joueur = document.getElementById(id);
                scoreJoueur[7].style.background="green";
                classement[7].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        /***********************************score 7 est le plus grand */
        else {
            id = "1" + tabJoueur[7];
            joueur = document.getElementById(id);
            for(let i=0;i<nombreJoueurA;i++)
            {
                scoreJoueur[i].style.background="";
                classement[i].innerHTML="";
            }
            scoreJoueur[7].style.background="green";
            classement[7].innerHTML="1er";
            if (typeof(finaliste1A) !== 'undefined')
            {
                finaliste1A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            if (score0 >= score1 && score0 >= score2 && score0 >= score3 && score0 >= score4 && score0 >= score5 && score0 >= score6) {
                id = "1" + tabJoueur[0];
                joueur = document.getElementById(id);
                scoreJoueur[0].style.background="green";
                classement[0].innerHTML="2nd";
            }
            else if (score1 >= score0 && score1 >= score2 && score1 >= score3 && score1 >= score4 && score1 >= score5 && score1 >= score6) {
                id = "1" + tabJoueur[1];
                joueur = document.getElementById(id);
                scoreJoueur[1].style.background="green";
                classement[1].innerHTML="2nd";
            }
            else if (score2 >= score0 && score2 >= score1 && score2 >= score3 && score2 >= score4 && score2 >= score5 && score2 >= score6) {
                id = "1" + tabJoueur[2];
                joueur = document.getElementById(id);
                scoreJoueur[2].style.background="green";
                classement[2].innerHTML="2nd";
            }
            else if (score3 >= score0 && score3 >= score1 && score3 >= score2 && score3 >= score4 && score3 >= score5 && score3 >= score6) {
                id = "1" + tabJoueur[3];
                joueur = document.getElementById(id);
                scoreJoueur[3].style.background="green";
                classement[3].innerHTML="2nd";
            }
            else if (score4 >= score0 && score4 >= score1 && score4 >= score2 && score4 >= score3 && score4 >= score5 && score4 >= score6) {
                id = "1" + tabJoueur[4];
                joueur = document.getElementById(id);
                scoreJoueur[4].style.background="green";
                classement[4].innerHTML="2nd";
            }
            else if (score5 >= score0 && score5 >= score1 && score5 >= score2 && score5 >= score3 && score5 >= score4 && score5 >= score6) {
                id = "1" + tabJoueur[5];
                joueur = document.getElementById(id);
                scoreJoueur[5].style.background="green";
                classement[5].innerHTML="2nd";
            }
            else {
                id = "1" + tabJoueur[6];
                joueur = document.getElementById(id);
                scoreJoueur[6].style.background="green";
                classement[6].innerHTML="2nd";
            }
            if (poolB.length == 1) 
            {
                finaliste2A.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2A.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2A.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
        }
        if (poolB.length == 1) 
        {
            finaliste1.innerHTML="";
            finaliste2.innerHTML="";
            finaliste3.innerHTML="";
            finaliste4.innerHTML="";
        }

        }
        
}

/*****************************************************************************pour la pool B */

if (poolB.length == 1)
{

/**pour les input de la pool B */
for (let b = 0; b < numJoueurB1.length; b++) {
    if (numJoueurB1[b].value != "")
    {
        numJoueurB1[b].style.background = "rgb(151, 238, 151)";
        numJoueurB1[b].setAttribute("class","numJoueurB numJoueurB1 score0 rempliB1");
    }  
    if (numJoueurB2[b].value != "")
    {
        numJoueurB2[b].style.background = "rgb(151, 238, 151)";
        numJoueurB2[b].setAttribute("class","numJoueurB numJoueurB2 score1 rempliB2");
    }  
    if (numJoueurB3[b].value != "")
    {
        numJoueurB3[b].style.background = "rgb(151, 238, 151)";
        numJoueurB3[b].setAttribute("class","numJoueurB numJoueurB3 score2 rempliB3");
    }
    if (numJoueurB4.length>0 && numJoueurB4[b].value != "")
    {
        numJoueurB4[b].style.background = "rgb(151, 238, 151)";
        numJoueurB4[b].setAttribute("class","numJoueurB numJoueurB4 score3 rempliB4");
    }
    if (numJoueurB5.length>0 && numJoueurB5[b].value != "")
    {
        numJoueurB5[b].style.background = "rgb(151, 238, 151)";
        numJoueurB5[b].setAttribute("class","numJoueurB numJoueurB5 score4 rempliB5");
    }
    if (numJoueurB6.length>0 && numJoueurB6[b].value != "")
    {
        numJoueurB6[b].style.background = "rgb(151, 238, 151)";
        numJoueurB6[b].setAttribute("class","numJoueurB numJoueurB6 score5 rempliB6");
    }
    if (numJoueurB7.length>0 && numJoueurB7[b].value != "")
    {
        numJoueurB7[b].style.background = "rgb(151, 238, 151)";
        numJoueurB7[b].setAttribute("class","numJoueurB numJoueurB7 score6 rempliB7");
    }  
    
}

rempliB1=document.getElementsByClassName("rempliB1");
rempliB2=document.getElementsByClassName("rempliB2");
rempliB3=document.getElementsByClassName("rempliB3");
rempliB4=document.getElementsByClassName("rempliB4");
rempliB5=document.getElementsByClassName("rempliB5");
rempliB6=document.getElementsByClassName("rempliB6");
rempliB7=document.getElementsByClassName("rempliB7");

if (rempliB1.length == nombreJoueurB && tourB[0].style.background != "rgb(3, 90, 3)") {
    tourB[0].style.background = "rgb(3, 90, 3)";
    infoTabB[0].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB2.length == nombreJoueurB && tourB[1].style.background != "rgb(3, 90, 3)") {
    tourB[1].style.background = "rgb(3, 90, 3)";
    infoTabB[1].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB3.length == nombreJoueurB && tourB[2].style.background != "rgb(3, 90, 3)") {
    tourB[2].style.background = "rgb(3, 90, 3)";
    infoTabB[2].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB4.length == nombreJoueurB && tourB[3].style.background != "rgb(3, 90, 3)") {
    tourB[3].style.background = "rgb(3, 90, 3)";
    infoTabB[3].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB5.length == nombreJoueurB && tourB[4].style.background != "rgb(3, 90, 3)") {
    tourB[4].style.background = "rgb(3, 90, 3)";
    infoTabB[4].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB6.length == nombreJoueurB && tourB[5].style.background != "rgb(3, 90, 3)") {
    tourB[5].style.background = "rgb(3, 90, 3)";
    infoTabB[5].style.border = "rgb(115, 255, 0) solid 2px";
}
if (rempliB7.length == nombreJoueurB && tourB[6].style.background != "rgb(3, 90, 3)") {
    tourB[6].style.background = "rgb(3, 90, 3)";
    infoTabB[6].style.border = "rgb(115, 255, 0) solid 2px";
}
if(tourB[0].style.background != "rgb(3, 90, 3)")
    numJoueurB1[(numJoueurB1.length-1)].focus();
else if (tourB[1].style.background != "rgb(3, 90, 3)")
    numJoueurB2[(numJoueurB1.length-1)].focus();
else if (tourB[2].style.background != "rgb(3, 90, 3)")
    numJoueurB3[(numJoueurB1.length-1)].focus();
else if (tourB.length>3 && tourB[3].style.background != "rgb(3, 90, 3)")
    numJoueurB4[(numJoueurB1.length-1)].focus();
else if (tourB.length>4 && tourB[4].style.background != "rgb(3, 90, 3)")
    numJoueurB5[(numJoueurB1.length-1)].focus();
else if (tourB.length>5 && tourB[5].style.background != "rgb(3, 90, 3)")
    numJoueurB6[(numJoueurB1.length-1)].focus();
else if (tourB.length>6 && tourB[6].style.background != "rgb(3, 90, 3)")
    numJoueurB7[(numJoueurB1.length-1)].focus();
scoreB1 = [];
scoreB2 = [];
scoreB3 = [];
scoreB4 = [];
scoreB5 = [];
scoreB6 = [];
scoreB7 = [];
totB = [];
for (i = 0; i < numJoueurB1.length; i++) {
    if (numJoueurB1[i].value != '')
        scoreB1[i] = parseInt(numJoueurB1[i].value);
    else
        scoreB1[i] = 0
}
for (i = 0; i < numJoueurB2.length; i++) {
    if (numJoueurB2[i].value != '')
        scoreB2[i] = parseInt(numJoueurB2[i].value);
    else
        scoreB2[i] = 0
}
for (i = 0; i < numJoueurB3.length; i++) {
    if (numJoueurB3[i].value != '')
        scoreB3[i] = parseInt(numJoueurB3[i].value);
    else
        scoreB3[i] = 0
}
for (i = 0; i < numJoueurB4.length; i++) {
    if (numJoueurB4[i].value != '')
        scoreB4[i] = parseInt(numJoueurB4[i].value);
    else
        scoreB4[i] = 0
}
for (i = 0; i < numJoueurB5.length; i++) {
    if (numJoueurB5[i].value != '')
        scoreB5[i] = parseInt(numJoueurB5[i].value);
    else
        scoreB5[i] = 0
}
for (i = 0; i < numJoueurB6.length; i++) {
    if (numJoueurB6[i].value != '')
        scoreB6[i] = parseInt(numJoueurB6[i].value);
    else
        scoreB6[i] = 0
}
for (i = 0; i < numJoueurB7.length; i++) {
    if (numJoueurB7[i].value != '')
        scoreB7[i] = parseInt(numJoueurB7[i].value);
    else
        scoreB7[i] = 0
}
 /******************************************************************************************************************************************************************************************************
             * ******************************************************************************************************************************score*/
            switch (nombreJoueurB) {
                /***************************************************************************************************************case 3 */
                case 3:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[2];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[0];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[1];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3=  0 ;
                        scoreB4=  0 ;
                        scoreB5=  0 ;
                        scoreB6=  0 ;
                        scoreB7=  0 ;
                    }
                    break;
                    /***************************************************************************************************************case 4 */
                case 4:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[0];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[2];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[3];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    totB[3] = scoreB1[3] + scoreB2[3] + scoreB3[1];
                    scoreTotal[(nombreJoueurA + 3)].innerHTML = totB[3];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3 = parseInt(scoreTotal[(nombreJoueurA + 3)].innerHTML);
                        scoreB4=  0 ;
                        scoreB5=  0 ;
                        scoreB6=  0 ;
                        scoreB7=  0 ;
                    }
                    break;
                    /***************************************************************************************************************case 5 */
                case 5:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[0] + scoreB4[0] + scoreB5[4];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[2] + scoreB4[4] + scoreB5[0];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[4] + scoreB4[2] + scoreB5[1];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    totB[3] = scoreB1[3] + scoreB2[4] + scoreB3[3] + scoreB4[1] + scoreB5[2];
                    scoreTotal[(nombreJoueurA + 3)].innerHTML = totB[3];
                    totB[4] = scoreB1[4] + scoreB2[3] + scoreB3[1] + scoreB4[3] + scoreB5[3];
                    scoreTotal[(nombreJoueurA + 4)].innerHTML = totB[4];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)" && tourB[3].style.background == "rgb(3, 90, 3)" && tourB[4].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3 = parseInt(scoreTotal[(nombreJoueurA + 3)].innerHTML);
                        scoreB4 = parseInt(scoreTotal[(nombreJoueurA + 4)].innerHTML);
                        scoreB5=  0 ;
                        scoreB6=  0 ;
                        scoreB7=  0 ;
                    }
                    break;
                    /***************************************************************************************************************case 6 */
                case 6:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[0] + scoreB4[0] + scoreB5[4];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[2] + scoreB4[4] + scoreB5[0];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[4] + scoreB4[2] + scoreB5[1];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    totB[3] = scoreB1[3] + scoreB2[4] + scoreB3[3] + scoreB4[1] + scoreB5[2];
                    scoreTotal[(nombreJoueurA + 3)].innerHTML = totB[3];
                    totB[4] = scoreB1[4] + scoreB2[3] + scoreB3[1] + scoreB4[3] + scoreB5[3];
                    scoreTotal[(nombreJoueurA + 4)].innerHTML = totB[4];
                    totB[5] = scoreB1[5] + scoreB2[5] + scoreB3[5] + scoreB4[5] + scoreB5[5];
                    scoreTotal[(nombreJoueurA + 5)].innerHTML = totB[5];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)" && tourB[3].style.background == "rgb(3, 90, 3)" && tourB[4].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3 = parseInt(scoreTotal[(nombreJoueurA + 3)].innerHTML);
                        scoreB4 = parseInt(scoreTotal[(nombreJoueurA + 4)].innerHTML);
                        scoreB5 = parseInt(scoreTotal[(nombreJoueurA + 5)].innerHTML);
                        scoreB6=  0 ;
                        scoreB7=  0 ;
                    }
                    break;
                    /***************************************************************************************************************case 7 */
                case 7:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[0] + scoreB4[0] + scoreB5[0] + scoreB6[0] + scoreB7[6];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[2] + scoreB4[4] + scoreB5[2] + scoreB6[6] + scoreB7[0];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[3] + scoreB4[2] + scoreB5[6] + scoreB6[2] + scoreB7[2];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    totB[3] = scoreB1[3] + scoreB2[4] + scoreB3[1] + scoreB4[6] + scoreB5[3] + scoreB6[4] + scoreB7[4];
                    scoreTotal[(nombreJoueurA + 3)].innerHTML = totB[3];
                    totB[4] = scoreB1[4] + scoreB2[3] + scoreB3[6] + scoreB4[1] + scoreB5[4] + scoreB6[3] + scoreB7[5];
                    scoreTotal[(nombreJoueurA + 4)].innerHTML = totB[4];
                    totB[5] = scoreB1[5] + scoreB2[6] + scoreB3[4] + scoreB4[5] + scoreB5[1] + scoreB6[5] + scoreB7[3];
                    scoreTotal[(nombreJoueurA + 5)].innerHTML = totB[5];
                    totB[6] = scoreB1[6] + scoreB2[5] + scoreB3[5] + scoreB4[3] + scoreB5[5] + scoreB6[1] + scoreB7[1];
                    scoreTotal[(nombreJoueurA + 6)].innerHTML = totB[6];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)" && tourB[3].style.background == "rgb(3, 90, 3)" && tourB[4].style.background == "rgb(3, 90, 3)"
                    && tourB[5].style.background == "rgb(3, 90, 3)" && tourB[6].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3 = parseInt(scoreTotal[(nombreJoueurA + 3)].innerHTML);
                        scoreB4 = parseInt(scoreTotal[(nombreJoueurA + 4)].innerHTML);
                        scoreB5 = parseInt(scoreTotal[(nombreJoueurA + 5)].innerHTML);
                        scoreB6 = parseInt(scoreTotal[(nombreJoueurA + 6)].innerHTML);
                        scoreB7=  0 ;
                    }
                    break;
                    /***************************************************************************************************************case 8 */
                case 8:
                    totB[0] = scoreB1[0] + scoreB2[0] + scoreB3[0] + scoreB4[0] + scoreB5[0] + scoreB6[0] + scoreB7[6];
                    scoreTotal[(nombreJoueurA + 0)].innerHTML = totB[0];
                    totB[1] = scoreB1[1] + scoreB2[2] + scoreB3[2] + scoreB4[4] + scoreB5[2] + scoreB6[6] + scoreB7[0];
                    scoreTotal[(nombreJoueurA + 1)].innerHTML = totB[1];
                    totB[2] = scoreB1[2] + scoreB2[1] + scoreB3[3] + scoreB4[2] + scoreB5[6] + scoreB6[2] + scoreB7[2];
                    scoreTotal[(nombreJoueurA + 2)].innerHTML = totB[2];
                    totB[3] = scoreB1[3] + scoreB2[4] + scoreB3[1] + scoreB4[6] + scoreB5[3] + scoreB6[4] + scoreB7[4];
                    scoreTotal[(nombreJoueurA + 3)].innerHTML = totB[3];
                    totB[4] = scoreB1[4] + scoreB2[3] + scoreB3[6] + scoreB4[1] + scoreB5[4] + scoreB6[3] + scoreB7[5];
                    scoreTotal[(nombreJoueurA + 4)].innerHTML = totB[4];
                    totB[5] = scoreB1[5] + scoreB2[6] + scoreB3[4] + scoreB4[5] + scoreB5[1] + scoreB6[5] + scoreB7[3];
                    scoreTotal[(nombreJoueurA + 5)].innerHTML = totB[5];
                    totB[6] = scoreB1[6] + scoreB2[5] + scoreB3[5] + scoreB4[3] + scoreB5[5] + scoreB6[1] + scoreB7[1];
                    scoreTotal[(nombreJoueurA + 6)].innerHTML = totB[6];
                    totB[7] = scoreB1[7] + scoreB2[7] + scoreB3[7] + scoreB4[7] + scoreB5[7] + scoreB6[7] + scoreB7[7];
                    scoreTotal[(nombreJoueurA + 7)].innerHTML = totB[7];
                    if (tourB[0].style.background == "rgb(3, 90, 3)" && tourB[1].style.background == "rgb(3, 90, 3)" && tourB[2].style.background == "rgb(3, 90, 3)" && tourB[3].style.background == "rgb(3, 90, 3)" && tourB[4].style.background == "rgb(3, 90, 3)"
                    && tourB[5].style.background == "rgb(3, 90, 3)" && tourB[6].style.background == "rgb(3, 90, 3)") {
                        scoreB0 = parseInt(scoreTotal[(nombreJoueurA + 0)].innerHTML);
                        scoreB1 = parseInt(scoreTotal[(nombreJoueurA + 1)].innerHTML);
                        scoreB2 = parseInt(scoreTotal[(nombreJoueurA + 2)].innerHTML);
                        scoreB3 = parseInt(scoreTotal[(nombreJoueurA + 3)].innerHTML);
                        scoreB4 = parseInt(scoreTotal[(nombreJoueurA + 4)].innerHTML);
                        scoreB5 = parseInt(scoreTotal[(nombreJoueurA + 5)].innerHTML);
                        scoreB6 = parseInt(scoreTotal[(nombreJoueurA + 6)].innerHTML);
                        scoreB7 = parseInt(scoreTotal[(nombreJoueurA + 7)].innerHTML);   
                    }
                    break;
            }
                        /***********************************************************************************************************************************************************************************************************************
             * *****************************************************************************************************************************comparaison des scores*/
            if (typeof(scoreB0) !== 'undefined') {
            /***********************************score 0 de la pool B est le plus grand */
            if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                id = "1" + tabJoueurB[0];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                classement[(nombreJoueurA+0)].innerHTML="1er";
                if (scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB1 && scoreB6 >= scoreB2 && scoreB6 >= scoreB3 && scoreB6 >= scoreB4 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id); 
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 1 de la pool B est le plus grand */
            else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                id = "1" + tabJoueurB[1];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                classement[(nombreJoueurA+1)].innerHTML="1er";
                if (scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB0 && scoreB6 >= scoreB2 && scoreB6 >= scoreB3 && scoreB6 >= scoreB4 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 2 de la pool B est le plus grand */
            else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                id = "1" + tabJoueurB[2];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                classement[(nombreJoueurA+2)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB0 && scoreB6 >= scoreB1 && scoreB6 >= scoreB3 && scoreB6 >= scoreB4 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);   
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd"; 
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 3 de la pool B est le plus grand */
            else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                id = "1" + tabJoueurB[3];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                classement[(nombreJoueurA+3)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB0 && scoreB6 >= scoreB1 && scoreB6 >= scoreB2 && scoreB6 >= scoreB4 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 4 de la pool B est le plus grand */
            else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                id = "1" + tabJoueurB[4];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                classement[(nombreJoueurA+4)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB0 && scoreB6 >= scoreB1 && scoreB6 >= scoreB2 && scoreB6 >= scoreB3 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);  
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 5 de la pool B est le plus grand */
            else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6 && scoreB5 >= scoreB7) {
                id = "1" + tabJoueurB[5];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                classement[(nombreJoueurA+5)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB6 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB6 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB6 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB6 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB6 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB6 >= scoreB0 && scoreB6 >= scoreB1 && scoreB6 >= scoreB2 && scoreB6 >= scoreB3 && scoreB6 >= scoreB4 && scoreB6 >= scoreB7) {
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 6 de la pool B est le plus grand */
            else if(scoreB6 >= scoreB0 && scoreB6 >= scoreB1 && scoreB6 >= scoreB2 && scoreB6 >= scoreB3 && scoreB6 >= scoreB4 && scoreB6 >= scoreB5 && scoreB6 >= scoreB7){
                id = "1" + tabJoueurB[6];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                classement[(nombreJoueurA+6)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB7) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB7) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB7) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB7) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB7) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB7) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[7];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                    classement[(nombreJoueurA+7)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            /***********************************score 7 de la pool B est le plus grand */
            else{
                id = "1" + tabJoueurB[7];
                joueur = document.getElementById(id);
                finaliste1B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste1B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste1B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
                for(let i=nombreJoueurA;i<(nombreJoueurA+nombreJoueurB);i++)
                {
                    scoreJoueur[i].style.background="";
                    classement[i].innerHTML="";
                }
                scoreJoueur[(nombreJoueurA+7)].style.background="#76761c";
                classement[(nombreJoueurA+7)].innerHTML="1er";
                if (scoreB0 >= scoreB1 && scoreB0 >= scoreB2 && scoreB0 >= scoreB3 && scoreB0 >= scoreB4 && scoreB0 >= scoreB5 && scoreB0 >= scoreB6) {
                    id = "1" + tabJoueurB[0];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+0)].style.background="#76761c";
                    classement[(nombreJoueurA+0)].innerHTML="2nd";
                }
                else if (scoreB1 >= scoreB0 && scoreB1 >= scoreB2 && scoreB1 >= scoreB3 && scoreB1 >= scoreB4 && scoreB1 >= scoreB5 && scoreB1 >= scoreB6) {
                    id = "1" + tabJoueurB[1];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+1)].style.background="#76761c";
                    classement[(nombreJoueurA+1)].innerHTML="2nd";
                }
                else if (scoreB2 >= scoreB0 && scoreB2 >= scoreB1 && scoreB2 >= scoreB3 && scoreB2 >= scoreB4 && scoreB2 >= scoreB5 && scoreB2 >= scoreB6) {
                    id = "1" + tabJoueurB[2];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+2)].style.background="#76761c";
                    classement[(nombreJoueurA+2)].innerHTML="2nd";
                }
                else if (scoreB3 >= scoreB0 && scoreB3 >= scoreB1 && scoreB3 >= scoreB2 && scoreB3 >= scoreB4 && scoreB3 >= scoreB5 && scoreB3 >= scoreB6) {
                    id = "1" + tabJoueurB[3];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+3)].style.background="#76761c";
                    classement[(nombreJoueurA+3)].innerHTML="2nd";
                }
                else if (scoreB4 >= scoreB0 && scoreB4 >= scoreB1 && scoreB4 >= scoreB2 && scoreB4 >= scoreB3 && scoreB4 >= scoreB5 && scoreB4 >= scoreB6) {
                    id = "1" + tabJoueurB[4];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+4)].style.background="#76761c";
                    classement[(nombreJoueurA+4)].innerHTML="2nd";
                }
                else if (scoreB5 >= scoreB0 && scoreB5 >= scoreB1 && scoreB5 >= scoreB2 && scoreB5 >= scoreB3 && scoreB5 >= scoreB4 && scoreB5 >= scoreB6) {
                    id = "1" + tabJoueurB[5];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+5)].style.background="#76761c";
                    classement[(nombreJoueurA+5)].innerHTML="2nd";
                }
                else{
                    id = "1" + tabJoueurB[6];
                    joueur = document.getElementById(id);
                    scoreJoueur[(nombreJoueurA+6)].style.background="#76761c";
                    classement[(nombreJoueurA+6)].innerHTML="2nd";
                }
                finaliste2B.innerHTML = joueur.attributes.prenomJoueur.value + " " + joueur.attributes.nomJoueur.value;
                finaliste2B.setAttribute("nomJoueur",joueur.attributes.nomJoueur.value);
                finaliste2B.setAttribute("prenomJoueur",joueur.attributes.prenomJoueur.value);
            }
            finaliste1.innerHTML="";
            finaliste2.innerHTML="";
            finaliste3.innerHTML="";
            finaliste4.innerHTML="";
   }


/***********************************************************************************************************************************************************************************************
 * *******************************************************************************************************************************************phase final */
    /*si on click sur le joueur qui a fini premier de la pool A*/ 
    finaliste1A.addEventListener("click", function () {
        finaliste1A = document.getElementById("1A");
        finaliste1AHtml = finaliste1A.innerHTML;
        finaliste2B = document.getElementById("2B");
        finaliste2BHtml = finaliste2B.innerHTML;
        finaliste1.innerHTML = finaliste1AHtml;
        finaliste1.setAttribute("nomJoueur",finaliste1A.attributes.nomJoueur.value);
        finaliste1.setAttribute("prenomJoueur",finaliste1A.attributes.prenomJoueur.value);
        finaliste3.innerHTML = finaliste2BHtml;
    });
    /*si on click sur le joueur qui a fini deuxieme de la pool B*/ 
    finaliste2B.addEventListener("click", function () {
        finaliste1A = document.getElementById("1A");
        finaliste1AHtml = finaliste1A.innerHTML;
        finaliste2B = document.getElementById("2B");
        finaliste2BHtml = finaliste2B.innerHTML;
        finaliste1.innerHTML = finaliste2BHtml;
        finaliste1.setAttribute("nomJoueur",finaliste2B.attributes.nomJoueur.value);
        finaliste1.setAttribute("prenomJoueur",finaliste2B.attributes.prenomJoueur.value);
        finaliste3.innerHTML = finaliste1AHtml;
    });
    /*si on click sur le joueur qui a fini premier de la pool B*/ 
    finaliste1B.addEventListener("click", function () {
        finaliste1B = document.getElementById("1B");
        finaliste1BHtml = finaliste1B.innerHTML;
        finaliste2A = document.getElementById("2A");
        finaliste2AHtml = finaliste2A.innerHTML;
        finaliste2.innerHTML = finaliste1BHtml;
        finaliste2.setAttribute("nomJoueur",finaliste1B.attributes.nomJoueur.value);
        finaliste2.setAttribute("prenomJoueur",finaliste1B.attributes.prenomJoueur.value);
        finaliste4.innerHTML = finaliste2AHtml;
    });
    /*si on click sur le joueur qui a fini deuxieme de la pool A*/ 
    finaliste2A.addEventListener("click", function () {
        finaliste1B = document.getElementById("1B");
        finaliste1BHtml = finaliste1B.innerHTML;
        finaliste2A = document.getElementById("2A");
        finaliste2AHtml = finaliste2A.innerHTML;
        finaliste2.innerHTML = finaliste2AHtml;
        finaliste2.setAttribute("nomJoueur",finaliste2A.attributes.nomJoueur.value);
        finaliste2.setAttribute("prenomJoueur",finaliste2A.attributes.prenomJoueur.value);
        finaliste4.innerHTML = finaliste1BHtml;
    });
    //si le gagnant est le finaliste 1
    finaliste1.addEventListener("click",function(){
        gagnant=document.getElementsByClassName("gagnant");
        gagnantNom=gagnant[0];
        gagnantPrenom=gagnant[1];
        gagnantIdEvenement=gagnant[2];
        gagnantNom.value=finaliste1.attributes.nomJoueur.value;
        gagnantPrenom.value=finaliste1.attributes.prenomJoueur.value;
    });
    //si le gagnant est le finaliste 2
    finaliste2.addEventListener("click",function(){
        gagnant=document.getElementsByClassName("gagnant");
        gagnantNom=gagnant[0];
        gagnantPrenom=gagnant[1];
        gagnantIdEvenement=gagnant[2];
        gagnantNom.value=finaliste2.attributes.nomJoueur.value;
        gagnantPrenom.value=finaliste2.attributes.prenomJoueur.value;
    });
}
/**si il n'y a qu'une pool */
if (poolB.length < 1) 
{
    selectionGagnant=document.getElementById("fin");
    selectionGagnant.addEventListener("click",function(){
        gagnant=document.getElementsByClassName("gagnant");
        gagnantNom=gagnant[0];
        gagnantPrenom=gagnant[1];
        gagnantIdEvenement=gagnant[2];
        score=document.getElementsByClassName("score");
        if ( classement[0].innerHTML=="1er")
        {
            score=score[0];
        }
        else if ( classement[1].innerHTML=="1er")
        {
            score=score[1];   
        }
        else if ( classement[2].innerHTML=="1er")
        {
            score=score[2];   
        }
        else if ( classement[3].innerHTML=="1er")
        {
            score=score[3];    
        }
        else if ( classement[4].innerHTML=="1er")
        {
            score=score[4];    
        }
        else if ( classement[5].innerHTML=="1er")
        {
            score=score[5];    
        }
        else if ( classement[6].innerHTML=="1er")
        {
            score=score[6];    
        }
        else if ( classement[7].innerHTML=="1er")
        {
            score=score[7];     
        }
        /**je recherche l'input caché et je le rempli */
        prenomGagnant=score.childNodes[2].nextElementSibling.innerHTML;
        nomGagnant=score.childNodes[4].nextElementSibling.innerHTML;
        gagnantNom.value=nomGagnant;
        gagnantPrenom.value=prenomGagnant;
    });
} 
